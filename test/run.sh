#!/usr/bin/env bash
# End-to-end test of Atlas's inbound-digest PRODUCER (bin/deliver) — including the
...
# Atlas keys: one age identity (ratchet resume) + one ssh signer (manifest head).
age-keygen -o "$work/atlas-id.txt" 2>/dev/null
export ATLAS_IDENTITY_FILE="$work/atlas-id.txt"
ssh-keygen -t ed25519 -N '' -C atlas-signer -f "$work/sign" >/dev/null
...
[ -f "$feed/inbox/seed.age" ] && [ -f "$feed/inbox/seed.atlas.age" ] || fail "genesis missing seed files"
[ "$(jq -r '.head.seq' "$feed/inbox/manifest.json")" = 1 ] || fail "genesis head seq != 1"
ok "genesis wrote seq 0..1 + owner/atlas seeds"

echo "[2] resume delivery in a FRESH process (ratchet replayed from seed.atlas.age)"
printf 'charlie\n' > "$work/b2"
...
if [ -x "$DP_REPO/bin/verify" ]; then
  ...
  printf 'atlas %s\n' "$(cat "$work/sign.pub")" > "$work/atlas.signers"
  "$DP_REPO/bin/verify" --dir "$feed" --source atlas --signers "$work/atlas.signers" >/dev/null \
  ...
  if "$DP_REPO/bin/verify" --dir "$work/tampered" --source atlas --signers "$work/atlas.signers" >/dev/null 2>&1; then
  ...
  DP_IDENTITY_FILE="$work/pile-id.txt" "$DP_REPO/bin/prove" --dir "$feed" --from 1 >/dev/null
  "$DP_REPO/bin/prove" --dir "$feed" --check "$feed/reports/proof-atlas-from-1.json" >/dev/null \
