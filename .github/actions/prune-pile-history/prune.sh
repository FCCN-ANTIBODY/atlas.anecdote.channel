#!/usr/bin/env bash
# Housekeeping for the "retain + periodic prune" custody choice. Pile branches are
# append-only signed-commit logs; left alone they grow forever. This bounds the LIVE
# ref while preserving the signed audit trail elsewhere.
#
...
BRANCH_PATTERN="${BRANCH_PATTERN:-pile/*}"
ARCHIVE_PREFIX="${ARCHIVE_PREFIX:-archive/}"
REMOTE="${REMOTE:-origin}"

git config user.name  "${GIT_USER_NAME:-atlas-prune}"
git config user.email "${GIT_USER_EMAIL:-atlas-prune@users.noreply.github.com}"
